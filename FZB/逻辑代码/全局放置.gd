...
func 加载存档() -> bool:
	...
	if file != null:
		...
		if parse_result == OK:
			...
			# 新增：检查并初始化键（兼容旧存档）
			var 存档修复次数 = 0
			if "植物数据" not in 放置板存档:
				放置板存档["植物数据"] = {}  # 创建空的植物数据字典
				存档修复次数+=1
				print("存档修复：添加植物数据")
			if "物品" not in 放置板存档:
				放置板存档["物品"] = {}  # 创建空的物品字典
...
func 保存存档() -> bool:
	...
	if file != null:
		...
		var json = JSON.new()
		var json_string = json.stringify(放置板存档)  # 这里是修复的地方
...
func 获取物品数量(item_name: String) -> int:
	return 放置板存档["物品"].get(item_name, 0)

# 设置物品数量
func 设置物品数量(item_name: String, count: int) -> bool:
	if 放置板存档["物品"].has(item_name):
		放置板存档["物品"][item_name] = count
		保存存档()
		return true
	return false
...
func 减少物品数量(item_name: String, 数量: int) -> bool:
	if 放置板存档["物品"].has(item_name) and 放置板存档["物品"][item_name] >= 数量:
		放置板存档["物品"][item_name] -= 数量
		保存存档()
		return true
	return false
...
func 通关(关卡名称: String) -> bool:
	...
	# 2. 通关次数+1
	放置板存档["关卡"][关卡名称]["通关次数"] += 1
	print("关卡 [", 关卡名称, "] 通关次数更新为: ", 放置板存档["关卡"][关卡名称]["通关次数"])
